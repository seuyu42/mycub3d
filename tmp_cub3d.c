...
// 2021.02.04 03:00지금 파싱한 맵이 세그펄트가 뜸.. 이유룰 찾아야함 ㅠ
#include "cub3d.h"




// int	spriteOrder[pars.sp_num];
// double spriteDistance[pars.sp_num];
...
void	draw(t_info *info)
{
	...
	while (y < W_H)
	{
		...
		while(x < W_W)
		{
			info->img.data[y * W_W + x] = info->buf[y][x];
			x++;
		}
		y++;
	}
...
void draw_bground(t_info *info)
{
	...
	while(++i < W_H)
	{
		...
		{
			if(i < vert)
				// info->buf[i][j] = 0x00a3d2;
				info->buf[i][j] = info->pars->ceil_clr;
			else
				// info->buf[i][j] = 0x9c6136;
...
//
		...
		//t_num이 아니라 동서남북에 따라서 다르게 그려야함.
		//동 ->  stepX = 1 (raydirX > 0) & x값에 부딪힘 (side = 0)
		//서 ->  stepX = -1 (radirX < 0) & x값에 부딪힘 (side = 0)
		//남 ->  stepY = -1 (radirY < 0) & y값에 부딪힘 (side = 1)
		//북 ->  stepY = 1 (raydirY > 0) & y값에 부딪힘 (side = 1)
	...
	while (++i < info->pars->sp_num)
	{
		...
		int spriteWidth = (int)fabs((W_W / transformY) / uDiv);
		...
		while (++stripe < drawEndX)
		{
			...
			if (transformY > 0 && stripe > 0 && stripe < W_W && transformY < info->z_buffer[stripe])
			{
				// int drawStartY = -spriteHeight / 2 + W_H / 2 + vMoveScreen;
				// if(drawStartY < 0) drawStartY = 0;
				int y = drawStartY - 1;
				while(++y < drawEndY)
				{
					...
					int d = (y - vMoveScreen) - (W_H / 2) + (spriteHeight / 2);
...
{
	if(!(info->buf = (int **)malloc(sizeof(int *) * W_H)))
		return ;
	int i = -1;
	while (++i < W_H)
	{
		if(!(info->buf[i] = (int *)malloc(sizeof(int) * W_W)))
			return ;
